<template>
  ...
  <el-dialog
    ...
    <el-form :model="form" ref="formRef" :rules="formRules" label-position="right" label-width="150px" >
      <div v-show="active === 1">
          ...
          <el-form-item label="Git凭据：">
            <el-select class="m-2" v-model="form.git_credential_id" @click="getCredential"  placeholder="请选择" style="width: 100%" clearable>
          ...
          <el-form-item label="消息通知：">
            <el-select class="m-2" v-model="form.notify_id" @click="getNotify"  placeholder="请选择" style="width: 100%" clearable>
              <el-option label="无" value="0"></el-option>
...
<script>
    export default {
        ...
        methods: {
            // 创建提交
            submit() {
                this.form.app = this.rowId; // 写入应用ID
                this.form.history_version_number = 7; // 默认7
                this.$refs.formRef.validate((valid) => {
                  if (valid) {
                    this.$http.post('/app_release/config/', this.form)
                    .then(res => {
