<template>
   <el-card>
       ...
       <el-table
               ...
               style="width: 100%"
       >
          <el-table-column prop="name" label="环境名称" fixed="left" sortable v-if="showColumn.name"/>
...
<envEdit v-model:visible="editDiglogVisible"  :row="currentRow"></envEdit>
<envCreate v-model:visible="createDiglogVisible"></envCreate>
</template>

<script>
    import envEdit from "@/views/app_env/envEdit";
    import envCreate from "@/views/app_env/envCreate";
    export default {
        name: "env",
        data() {
          return {
        ...
        methods: {
            getData() {
                this.$http.get('/app_release/env/', {params: this.urlParams})
                .then(res => {
            ...
            Delete(index, row) {
                this.$confirm("你确定要删除这条记录吗？", "提示", {
                  ...
                  .then(() => {  // 点击确定
                    this.$http.delete('/app_release/env/'+ row.id + '/')
                        .then(res => {
